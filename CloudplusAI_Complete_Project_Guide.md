...
```
apt update -y
apt install nodejs npm -y              install command
node -v                 ckeck the instaletion
npm -v---                     version check command
...
```text
MongoDB is not required for this Groq API chatbot.
So we are not using mongo in final setup.
```
